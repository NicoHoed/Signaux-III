...
Reconnaissance AZERTY vs QWERTY basée sur la position (Amélioration) :


Le code actuel utilise l'Extent et la position Y normalisée de la touche TL_LETTER pour distinguer QWERTY/AZERTY. C'est une bonne approche par heuristique géométrique (le QWERTY "Q" est plus haut que le AZERTY "A").

Proposition : Ajouter un critère basé sur la position en X de la touche "L" ou "M" par rapport à l'axe central du clavier, ce qui est une signature forte des layouts français/belge.


---


Détection de l'Entrée ISO/ANSI (Amélioration) :


Le format ISO est uniquement inféré à partir du petit Shift gauche.

Proposition : Renforcer la classification en vérifiant l'existence d'une touche Entrée ISO (souvent un grand L inversé, donc avec un grand ratio hauteur/largeur pour sa forme globale).
...
Le projet manque de tests unitaires, ce qui est essentiel pour la robustesse des systèmes de Computer Vision qui reposent sur des seuils.

4.1 Tests Unitaires (Module unittest ou pytest)

Je recommanderais l'utilisation de pytest pour sa simplicité.
...
On ne fera pas de machine learning dans ce projet.